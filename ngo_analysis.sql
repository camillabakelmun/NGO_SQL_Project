...
--max population - China - 1439323776
SELECT name, population
FROM countries
WHERE population = (SELECT MAX(population) FROM countries);

--min population - Antigua and Barbuda - 97929
SELECT name, population
FROM countries
...
-- average population - 38774455
SELECT ROUND(AVG(population), 0) AS 'average_population'
FROM countries;

-- Average, max, and min population
SELECT ROUND(AVG(population),0) AS avg_population, ROUND(MAX(population),0) AS max_population, ROUND(MIN(population),0) AS min_population
FROM countries;

-- Average fertility rate and max/min by country
SELECT ROUND(AVG(fertility_rate),0) AS avg_fertility, MAX(fertility_rate) AS max_fertility, MIN(fertility_rate) AS min_fertility
FROM countries;

-- Density per square kilometer statistics
SELECT ROUND(AVG(density_per_sq_km),0) AS avg_density, ROUND(MAX(density_per_sq_km),0) AS max_density, ROUND(MIN(density_per_sq_km),0) AS min_density
FROM countries;

-- Average percent of world population represented by a country
SELECT ROUND(AVG(percent_of_world_pop),2) AS avg_percent_pop, ROUND(MAX(percent_of_world_pop),2) AS max_percent_pop, ROUND(MIN(percent_of_world_pop),2) AS min_percent_pop
FROM countries;

-- Average, max, and min net migrants
SELECT ROUND(AVG(net_migrants),0) AS avg_net_migrants, ROUND(MAX(net_migrants),0) AS max_net_migrants, ROUND(MIN(net_migrants),0) AS min_net_migrants
FROM countries;

-- Countries with above-average density
SELECT 
    name, 
    ROUND(density_per_sq_km, 2)
FROM countries
GROUP BY name
HAVING ROUND(AVG(density_per_sq_km), 2) > (SELECT ROUND(AVG(density_per_sq_km), 2) FROM countries)
LIMIT 5;

-- Names with above-average density
SELECT 
    name, 
    ROUND(density_per_sq_km, 2)
FROM countries
...
-- Names grouped by high or low fertility rate
SELECT 
    name, 
    ROUND(fertility_rate, 2)
FROM countries
GROUP BY fertility_rate
HAVING ROUND(AVG(fertility_rate), 2) > 2.1
LIMIT 5;

-- Names with significant population changes
SELECT 
    name, 
    ROUND(population_change, 2)
FROM countries
GROUP BY name
HAVING ROUND(MAX(population_change), 2) > 1000000
LIMIT 5;

-- Names with above-average area size
SELECT 
    name, 
    ROUND(area_sq_km, 2)
FROM countries
GROUP BY area_sq_km
HAVING ROUND(AVG(area_sq_km), 2) > (SELECT ROUND(AVG(area_sq_km), 2) FROM countries)
LIMIT 5;

-- Names contributing significantly to world population
SELECT 
    name, 
    ROUND(percent_of_world_pop, 2)
FROM countries
GROUP BY percent_of_world_pop
HAVING ROUND(SUM(percent_of_world_pop), 2) > 5
LIMIT 5;

-- Classify names by population size
SELECT 
    name,
    CASE
...
-- Classify names by population density
SELECT 
    name,
    CASE
...
-- Classify names by migration trends
SELECT 
    name,
    CASE
...
-- Categorize names by population growth rate
SELECT 
    name,
    CASE
...
-- Names with high population density and low fertility
SELECT 
    name, 
    ROUND(density_per_sq_km, 2), 
    ROUND(fertility_rate, 2)
...
-- Names with population growth and high migration
SELECT 
    name, 
    ROUND(percent_one_year_change, 2), 
    ROUND(net_migrants, 2)
FROM countries
WHERE percent_one_year_change > 1 AND net_migrants > 10000
LIMIT 5;

-- Names with low population and high area size
SELECT 
    name, 
    ROUND(population, 2), 
    ROUND(area_sq_km, 2)
...
-- Names with high fertility and young median age
SELECT 
    name, 
    ROUND(fertility_rate, 2), 
    ROUND(median_age, 2)
...
-- Names contributing significantly to world population but experiencing decline
SELECT 
    name, 
    ROUND(percent_of_world_pop, 2), 
    ROUND(percent_one_year_change, 2)
